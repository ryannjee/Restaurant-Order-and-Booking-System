...
$total_query = "SELECT COUNT(*) as total FROM users_ordersin";
                    ...
                    <div class="col-lg-12">
                        <div class="card card-outline-primary">
                            <div class="search-container" style="display: flex; justify-content: space-between; align-items: center;">
                                <input type="text" id="searchInput" placeholder="&#xf002;  Search for dine-in orders ..." style="font-family:Arial, FontAwesome; margin-bottom: 10px;">  
                                <button name="search" class="btn btn-purple oval-button" style="margin-left: auto;" onclick="redirectToPayment()">Payment</button>                                                     
                            </div>
                            <br>
                                    ...
                                    <table id="myTable" class="table table-bordered table-striped table-hover" style="width:100%">
                                        ...
                                        <tbody id="tableBody">                                           											
											<?php
                                                ...
                                                ORDER BY users_ordersin.date DESC LIMIT $offset, $records_per_page";                                        
												$query=mysqli_query($db,$sql);												
												if (!mysqli_num_rows($query) > 0) {
                                                    echo '<td colspan="13"><center>No Orders</center></td>';
                                                } else {
                                                    while ($rows = mysqli_fetch_array($query)) {
                                                        ...
                                                        <tr>
                                                            ...
                                                            if ($status == 'Pay at Counter' || $status == '' || $status == 'NULL') {
                                                                ?>
                                                                <td> <button type="button" class="btn btn-primary"><span class="fa fa-dollar" aria-hidden="true"></span> Pay at Counter</button></td>
                                                            <?php } ?>
                                                            <?php if ($status == "Order Received") { ?>
                                                                <td> <button type="button" class="btn btn-info"><span class="fa fa-bars" aria-hidden="true"></span> Order Received</button></td>
                                                            <?php } ?>
                                                            <?php if ($status == "Preparing Your Order") { ?>
                                                                <td> <button type="button" class="btn btn-warning"><span class="fa fa-cog fa-spin" aria-hidden="true"></span> Preparing Your Order</button></td>
                                                            <?php } ?>
                                                            <?php if ($status == "Order Served") { ?>
                                                                <td> <button type="button" class="btn btn-success"> <i class="fa fa-check-circle"></i> Order Served</button></td>
                                                            <?php } ?> 
                                                            ...
                                                            <td>
                                                                <div class="btn-group" role="group">
                                                                    <a href="view_orderdinein.php?user_upd=<?php echo $rows['o_id']; ?>" class="btn btn-info btn-flat btn-addon btn-sm m-b-10 m-l-5"><i class="fa fa-edit"></i></a>
        ...
        input.addEventListener("keyup", function () {
            ...
            if (found || filter === "") {
                ...
                if (!noUsersRow) {
                    ...
                    noUsersCell.setAttribute("colspan", "14");
